...
struct lttng_save_session_attr *lttng_save_session_attr_create(void)
{
	...
	attr = zmalloc(sizeof(struct lttng_save_session_attr));
	return attr;
}
...
const char *lttng_save_session_attr_get_configuration_url(
	struct lttng_save_session_attr *attr)
{
	const char *ret = NULL;
...
int lttng_save_session_attr_get_configuration_overwrite(
	struct lttng_save_session_attr *attr)
{
	...
	struct lttng_save_session_attr *attr, const char *session_name)
{
	...
	if (session_name) {
		...
		strcpy(attr->session_name, session_name);
	} else {
...
int lttng_save_session_attr_set_configuration_url(
	struct lttng_save_session_attr *attr, const char *url)
{
	...
	if (url) {
		...
		strcpy(attr->configuration_url, url);
	} else {
